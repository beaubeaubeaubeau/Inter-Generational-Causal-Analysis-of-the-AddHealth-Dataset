...
- **A11.** **[P1] Sibling/twin within-school dependence — only option (a) feasible in public-use.** **Verified 2026-04-26:** the W1 in-home file ships sibling/twin FLAGS (`STUDSIBA-G`, `TWINA-G`) but **no pair-linking ID** (PAIRID/FAMID is restricted-use). ~13 % of W1 in-home respondents (826 / 6,504) have at least one study-sibling flag; most are likely co-school (absorbed by `CLUSTER2`). Only option (a) "document the assumption + cite the empirical % affected" is feasible without restricted-use access. Action: add a paragraph to `methods.md §3` stating the assumption ("sibling correlation is assumed absorbed by `CLUSTER2`; 13 % upper bound on within-household pairs") and a sensitivity-row caveat in handoff reports.
...
- ~~**A16.** **[P1] [JUDGMENT REQUIRED] D3 sibling-dissociation pairs.**~~ **Resolved 2026-04-26 (drop D3 for network exposures):** in `experiments/cognitive-screening/run.py`, D3 is now skipped for any exposure with `req_sat=True` (the 16 network-file-derived measures); `pass: None` so the classifier excludes it from the score. methods.md §2 D3 row updated. Rationale: no structurally-orthogonal placebo exists within the network file; D2 + D7 carry confounding-detection.
...
- ~~**A18.** **[P1] [JUDGMENT REQUIRED] D4 30% threshold computed against post-AHPVT β as denominator.**~~ **Resolved 2026-04-26 (split D4 into D4a + D4b):** D4 is now two diagnostics. **D4a (sensitivity, drives pass/fail):** rel-shift L0 → L0+L1 only — detects hidden confounding via W1 health/affect. Threshold `< 0.30` AND sign stable. **D4b (estimand presentation, no pass/fail):** records `β_levels` (L0+L1) vs `β_trajectory` (L0+L1+AHPVT) side-by-side as two distinct estimands; reports `abs_shift` and `pct_attenuation`. The L0+L1 → L0+L1+AHPVT step was a deliberate change of estimand (level → trajectory), not a sensitivity check; conflating them mathematically inflated rel-shift when the trajectory β was small. Implementation: `_d4_adj_stability` in cog-screening + `d4_outcome` in multi-outcome; classifier uses `d4_a_pass`; methods.md §2 split into D4a + D4b rows.
...
- **A23.** **[P0] L_partner placeholder for cognitive experiments — update when partner pushes refined covariate list.** Currently `L_partner = {BIO_SEX, H1GI1M}` mirroring partner's `dataset_eda.ipynb` Cell 21 as of 2026-04-27. Partner's notebook is a skeleton (one causal_df cell, no analysis run); they're expected to push updated covariate list soon. **When they do**, update the single `L_partner` definition (currently inline in each cognitive experiment's `run.py`; consider extracting to `scripts/analysis/adjustment_sets.py` if it grows) and re-run the cognitive experiments and `cognitive-screening`. Tracked owner: jb (and partner). Cross-references: [`experiments/cognitive-screening/dag.md`](experiments/cognitive-screening/dag.md), [`reference/dag_library.md`](reference/dag_library.md) DAG-Cog row.
- **A24.** **[P1] Phase 6 mechanism experiments scaffolded 2026-04-27 — implement run.py analyses end-to-end.** 9 new experiments registered in [experiments/README.md](experiments/README.md): popularity-vs-sociability, ego-network-density, friendship-quality-vs-quantity, em-compensatory-by-ses, em-sex-differential, em-depression-buffering, popularity-and-substance-use, lonely-at-the-top, cross-sex-friendship. Each ships dag.md + README.md + run.py skeleton + report.md template. Phase 5 of the master plan (`/Users/jb/.claude/plans/users-jb-desktop-causal-inference-midte-refactored-sutton.md`) is the per-experiment implementation phase.
...
- ~~Baked-in titles stripped from PNGs embedded in `research_journal.md`.~~ **Done 2026-04-22.** **Reversed 2026-04-26:** the convention now requires titles ON the chart (baked in via `ax.set_title` / `fig.suptitle`); see updated chart-explanation convention in README.md / experiments/README.md.
