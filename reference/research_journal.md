...
**Phase 6 execution (2026-04-27 evening)** — three parallel agents executed all 9 mechanism `run.py` scripts end-to-end. Bugs fixed mid-flight: (1) added 8 substance-use + 7 derived-variable + `H1GI1M` + `CESD_SUM` + `PARENT_ED` + `POLYSOCIAL_PC1` entries to `scripts/analysis/cleaning.py:VALID_RANGES` (cross-cutting fix avoided per-experiment patches); (2) merged `H1GI1M`, all W4 cardiometabolic, all W5 outcomes, all substance-use, and `GSW5` into `cache/analytic_w4.parquet` and `cache/analytic_w5.parquet` proactively (lesson: `feedback_proactive_cross_cutting_fixes`); (3) substance-use legitimate-skip code 97 → impute 0 (never-used = zero exposure) before the (0,6) reserve-code gate. **Findings**: 9 new claims (C5-C13) added to top-level [report.md](../report.md). Headline patterns: send-side beats receive-side on SES (C5-C7); cardiometabolic protection is sex-differential (C8); dark side is alcohol-only (C11); compensatory-by-SES, depression-buffering, lonely-at-top paradox, structural-holes prediction, and cross-sex emotional/instrumental hypotheses all NULL (C9, C10, C12, C13). All claims provisional pending handoff-style formal estimation. 229 tests + 1 xfailed throughout.
