...
Catalogued the 50 public-use files shipped with Add Health (see [addhealth_synthesis.md §1](addhealth_synthesis.md#1-scope-and-access) for distribution details) and built an AID-by-wave overlap matrix to establish analytic ceilings. W1 in-home N = 6,504; W1 in-school N = 20,745. W1 → W4 retention 4,882 (75 % of W1 in-home). W1 → W5 retention 4,196 mode-unrestricted / 3,553 flagged high-quality by `MODEOK5`.
...
**Tasks:** 03a/b/c/de/fgh, 04, 05. **Scripts:** [task03*](../scripts/), [task04_missingness.py](../scripts/task04_missingness.py), [task05_weighted_descriptives.py](../scripts/task05_weighted_descriptives.py).
...
Reserve-code handling (6/7/8/9, 96/97/98/99, 995/9995) is codified in `analysis_utils.VALID_RANGES` + `clean_var`. Survey-weighted descriptives use `GSWGT4_2` + `CLUSTER2`.
...
**Main finding.** Primary analytic frame N = 3,238 under the tightest restriction (mode-OK W4 cognitive + full covariate vector). Network-frame gating costs ~32 % of W1 in-home respondents — a structural saturation-sampling artefact, not attrition; documented as the task14 D8 column ("saturated-school selection penalty").
...
1. **`PRXPREST` mis-classified as `binary`** (task14 line 78). Raw dtype is float64 over [0.00045, 0.774] with 3,920 unique values across N = 4,020 non-null. D6 was being skipped and D7 ran a degenerate logit. Root cause: contradiction between [addhealth_synthesis.md §3.1](addhealth_synthesis.md#31-wave-i-public-use-network-file--pre-computed-sociometric-measures) (correct, continuous range) and pitfall #7 (then incorrect, "binary 0/1"). Synthesis was corrected, `kind` retagged to `continuous`, task14 re-run. Post-fix: D6 trend_ρ = 0.8 (PASS), D7 overlap [0.15, 0.78], eff_N = 1,204 (PASS). Re-categorised as **Weakened** (score 4). Logged in the synthesis [Changelog](addhealth_synthesis.md#changelog) dated 2026-04-20.
...
**Task:** 15. **Script:** [task15_multi_outcome.py](../scripts/task15_multi_outcome.py). **Outputs:** [outputs/15_multi_outcome.md](../outputs/15_multi_outcome.md), [outputs/15_multi_outcome_matrix.csv](../outputs/15_multi_outcome_matrix.csv).

Motivated by the observation that a cognition-only outcome over-weights the AHPVT-mediation path. Extended the outcome-dependent diagnostics (D1 baseline, D4 stability) across 12 non-cognitive outcomes in four groups: cardiometabolic (`H4BMI`, `H4WAIST`, `H4SBP`, `H4DBP`, `H4BMICLS`), mental health (`H5MN1`, `H5MN2`), functional (`H5ID1`, `H5ID4`, `H5ID16`), SES (`H5LM5`, `H5EC1`). D2 / D6 / D7 / D8 / D9 are outcome-agnostic and inherited from the Task 14 re-run; D3 (sibling) and D5 (component consistency) remain cognition-only.
...
1. ~~**AHPVT's role** is still ambiguous — confounder vs. mediator of social-exposure → outcome.~~ **Resolved 2026-04-25:** AHPVT serves as the W1 baseline cognitive measure; cognitive-outcome estimates are reported as **trajectory-adjusted** (change-from-baseline), with the construct-mismatch and pre-W1-mediation caveats kept in the [synthesis §5.6 callout](addhealth_synthesis.md#56-identification-assumptions-and-target-estimand). Task 16 front-door decomposition is now a **sensitivity check** quantifying how much the trajectory β shifts under the strict mediator reading, not a load-bearing alternative model.
...
- [scripts/task15_multi_outcome.py](../scripts/task15_multi_outcome.py), [scripts/task15_journal_figs.py](../scripts/task15_journal_figs.py).
- [outputs/15_multi_outcome.md](../outputs/15_multi_outcome.md), [outputs/15_multi_outcome_matrix.csv](../outputs/15_multi_outcome_matrix.csv) (288 rows).
- [img/causal/multi_outcome_beta_heatmap.png](../img/causal/multi_outcome_beta_heatmap.png), [multi_outcome_sig_heatmap.png](../img/causal/multi_outcome_sig_heatmap.png), [15_per_outcome_pcount.png](../img/causal/15_per_outcome_pcount.png), [15_handoff_forest.png](../img/causal/15_handoff_forest.png).
- [reference/research_journal.md](research_journal.md) (this file).

**Modified:**
- [scripts/task14_causal_screening.py](../scripts/task14_causal_screening.py) — 2-line D3 bug fix (sign-agreement check) + `PRXPREST` kind re-tag.
- [reference/addhealth_synthesis.md](addhealth_synthesis.md) — pitfall #7 rewrite (PRXPREST), §9 rewritten as outcome-battery, §10 extended through Task 15, variable-dictionary pointer, Changelog seeded.
...
- 5 figures in [img/causal/](../img/causal/) other than the two multi-outcome heatmaps and the two new journal figures.
