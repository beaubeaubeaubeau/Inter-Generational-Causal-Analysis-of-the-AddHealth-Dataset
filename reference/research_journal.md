...
1. **AHPVT's role** is still ambiguous — confounder vs. mediator of social-exposure → outcome. Plan for Task 16: front-door / IV decomposition with AHPVT as the mediator, compared to the current covariate-adjusted estimate, to bound the true effect.
...
6. **School-level saturation.** Task 14's D8 is informational only. Network-frame respondents are drawn from saturated schools (≥ 75 % roster participation), which differ systematically from the full W1 sample on urbanicity and school size. Formal estimation should stratify or weight on saturation.
