...
Produces:
  scripts/prep/figures/distributions/
...
# scripts/prep/ is this file's parent; scripts/ is its grandparent (which holds
...
from analysis.plot_style import setup, annotate_n, weighted_mean, weighted_median  # noqa: E402
from analysis import CACHE  # noqa: E402
from analysis.cleaning import clean_var  # noqa: E402

# Distribution figures are scoped to the prep pipeline; write them under
# scripts/prep/figures/ so the prep step is self-contained. We define a local
# `save` rather than monkey-patching plot_style.IMG, since the experiment
# scripts (task10-15) still rely on plot_style.IMG = ROOT/img for now.
FIG_ROOT = HERE / "figures"


def save(fig: plt.Figure, rel_path: str) -> Path:
    """Save figure under FIG_ROOT / rel_path. Mirrors plot_style.save signature."""
    path_png = FIG_ROOT / rel_path
...
def run() -> None:
    ...
    print(f"Done. Plots in {FIG_ROOT}/distributions/")
